...
const BundleItem: React.FC<{ bundle: Bundle; showId: string }> = ({
  bundle,
  showId,
}) => {
  ...
  // Use fileId for bundles. Assuming useStreamToken or backend handles fileId if provided.
  const { streamUrl } = useStreamToken({
    contentId: showId,
    mediaType: 'show',
    fileId: bundle.file_id, // Pass fileId for bundles
  ...
  const displayTitle = bundle.episode_range 
    ? `${bundle.episode_range} Pack`
    : "Season Pack";

  return (
    <motion.div
      whileHover={{ backgroundColor: "rgba(55, 55, 55, 0.4)" }}
      className="flex flex-col sm:flex-row items-center p-3 sm:p-4 border-b border-gray-800 relative group bg-gray-800/20 mb-2 rounded-lg"
    >
      <AnimatePresence>
        {showDownload && (
          <Download
      ...
      <div className="flex-shrink-0 relative w-12 h-12 mb-2 sm:mb-0 sm:mr-4 flex items-center justify-center bg-blue-900/30 rounded-lg text-blue-400">
      ...
      <div className="flex-grow w-full sm:w-auto pr-20 sm:pr-24">
        ...
        <p className="text-gray-400 text-xs line-clamp-1 mt-1 font-mono">
          {bundle.title}
        </p>
        <div className="flex gap-2 mt-2">
            <span className="text-xs bg-blue-900/50 text-blue-300 px-2 py-0.5 rounded border border-blue-900/50">
                Bundle
            </span>
            {bundle.note && (
...
const EpisodeList: React.FC<EpisodeListProps & { showId: string; currentSeason: number }> = ({
  ...
  if (!episodes || episodes.length === 0) {
    // If no episodes, just return null, the bundle list might be present
    return null;
  }
...
// Main Component
const TVShowEpisodes: React.FC<TVShowEpisodesProps> = ({ seasons, showId, bundles }) => {
  const [currentSeasonIndex, setCurrentSeasonIndex] = useState(0);
  const [searchQuery, setSearchQuery] = useState("");

  const seasonNumbers = seasons?.map(season => season.season_number) || [];
  
  // If no seasons but we have bundles, we can extract seasons from bundles or just default to 1
  if (seasonNumbers.length === 0 && bundles && bundles.length > 0) {
      // Create a set of unique seasons from bundles
      const bundleSeasons = Array.from(new Set(bundles.map(b => b.season))).sort((a,b) => a-b);
      if (bundleSeasons.length > 0) {
          seasonNumbers.push(...bundleSeasons);
      } else {
          seasonNumbers.push(1);
      }
  }

  const currentSeason = seasonNumbers[currentSeasonIndex] || 1;

  // Handle cases where season data exists
  const currentEpisodes = seasons?.find(s => s.season_number === currentSeason)?.episodes || [];
  
  // Filter bundles for current season
  const currentBundles = bundles?.filter(b => b.season === currentSeason) || [];

  // If absolutely no data
  if ((!seasons || seasons.length === 0) && (!bundles || bundles.length === 0)) {
    return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        {currentBundles.length > 0 && (
            <div className="mb-6">
                <h3 className="text-md font-semibold text-gray-400 mb-3 px-1 uppercase tracking-wider text-xs">
                    Season Packs & Bundles
                </h3>
                <div className="grid gap-2">
                    {currentBundles.map((bundle, idx) => (
                        <BundleItem key={idx} bundle={bundle} showId={showId} />
                    ))}
                </div>
            </div>
        )}
        ...
        {currentEpisodes.length === 0 && currentBundles.length === 0 && (
             <div className="text-center py-8 text-gray-400">
