...
export default async function handler(request: Request) {
  ...
  try {
    ...
    // LOGIC UPDATE: Handle Bundles using File Hash
    // Note: getShowDetails now sends the file_hash as 'fileId' to the frontend.
    if (isBundle && fileId) {
        // fileId passed here is actually the short file_hash from the backend
        telegramLink = `https://t.me/${NEXT_PUBLIC_TELEGRAM_BOT_NAME}?start=bundle_${fileId}`;
    } else {
        // Standard Movie/Show Logic
        ...
        telegramLink = `https://t.me/${NEXT_PUBLIC_TELEGRAM_BOT_NAME}?start=file_${compactParams}`;
    }
