...
export default async function handler(request: Request) {
  ...
  try {
    ...
    const { id, fileId } = decodedToken;

    // Prioritize fileId if available (for bundles/direct files), otherwise use content ID
    const targetId = fileId || id;

    const apiUrl = `${BACKEND_URL}/api/v1/dl/${targetId}?token=${token}`;
